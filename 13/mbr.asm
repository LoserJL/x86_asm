    ...
    flush:
        ...
        dec eax                         ;因为已经读取了一个扇区，所以要减1
    @1:
        or eax,eax
        jz setup                        ;EAX=0?，如果为0，则说明已经读取完成了

        ;读取剩余的扇区
        mov ecx,eax
        mov eax,core_start_sector
        inc eax                         ;因为已经读了一个扇区，所以从下一个扇区继续读
    @2:
    ...
    setup:
        mov esi,[0x7c00+pgdt+0x02]      ;通过0~4GB的数据段来寻址pgdt
        ...
        sub ebx,eax
        dec ebx                         ;公用例程段界限
        add eax,edi                     ;公用例程段基地址（公共例程段起始汇编地址加上内核的加载地址）
...
make_gdt_descriptor:
        ...
        rol edx,8                   ;循环左移
        bswap edx                   ;edx字节次序变反，装配基址的31~24和23~16 （80486+）
